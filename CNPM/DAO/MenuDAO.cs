...
namespace CNPM.DAO
{
    public class MenuDAO
    {
        ...
        public bool AddDrinks(string tenDoUong, int maDanhMuc, float giaBan, int tinhTrang)
        {
            string query = "insert into dbo.thucdon (madanhmuc,tendouong,giaban,tinhtrang) values ( @madanhmuc , @tendouong , @giaban , @tinhtrang )";
            int result = DataProvider.Instance.ExecuteNonQuery(query, new object[] { maDanhMuc, tenDoUong, giaBan, tinhTrang });
            return result > 0;
        }
